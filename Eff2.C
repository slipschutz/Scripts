...
void Eff2(TH1F * hN=0){
  ...
  /*  flt->Draw("TOFEnergy>>N","channels[0]==0 && NumOfChannelsInEvent==3&&channels[1]==1 && channels[2]==9&&longGates[2]/shortGates[2]<1.12&&0.5*(cubicTimes[0]+cubicTimes[1])-cubicTimes[2]>5");
  flt->Draw("TOFEnergy>>N2","channels[0]==0 && NumOfChannelsInEvent==3&&channels[1]==1 && channels[2]==9&&longGates[2]/shortGates[2]<1.12&&0.5*(cubicTimes[0]+cubicTimes[1])-cubicTimes[2]>5");
  */
  /*
  ...
  for (int i=0;i<=200;i++){
    Double_t energy = hN->GetBinCenter(i+1);
    Double_t counts = hN->GetBinContent(i+1);
    int bin= N->FindBin(energy);
     
    N->SetBinContent(bin,N->GetBinContent(bin)+counts);
  ...
  for (int i=0;i<71;i++){
    ...
    Double_t E=N->GetBinCenter(i+1);
    Double_t effN = 0.115121+0.497964*E-0.340233*E*E+0.0748143*E*E*E-0.00542478*E*E*E*E;
    eff->SetBinContent(i+1,Double_t((N->GetBinContent(i+1)))/(Yield->GetBinContent(i+1)));
    //    eff->SetBinContent(i+1,N->GetBinContent(i+1)/effN);
    cout<<N->GetBinContent(i+1)<<"  over "<<1000000*Yield->GetBinContent(i+1)<<endl;
  }
